...
async def add_blog(
    ...
    try:
        blog = await add_blog_to_bd(session=session, values=BlogCreateSchemaAdd.model_validate(blog_dict))
        blog_id = blog.id

        if tags:
            tags_ids = await add_tags_to_bd(session=session, tag_names=tags)
            await add_blog_tags_to_bd(session=session,
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Ошибка при добавлении блога. ERROR: {e}")
